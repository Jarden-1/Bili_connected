...
export function assertAllowedOriginsStartupPolicy(
  ...
  if (
    ...
    throw new SecurityConfigError(
      "ALLOWED_ORIGINS is empty; set ALLOW_MISSING_ORIGIN_IN_DEV=true to run without origin restrictions in development, set ALLOW_ANY_FIREFOX_EXTENSION_ORIGIN=true to accept any Firefox extension origin, set ALLOW_ANY_CHROME_EXTENSION_ORIGIN=true to accept any Chrome extension origin, or configure ALLOWED_ORIGINS for production.",
    );
  }
}
...
export function logEffectiveOriginPolicy(
  ...
  log(
    `[security] ALLOWED_ORIGINS=${origins}; ALLOW_MISSING_ORIGIN_IN_DEV=${String(config.allowMissingOriginInDev)}; ALLOW_ANY_FIREFOX_EXTENSION_ORIGIN=${String(config.allowAnyFirefoxExtensionOrigin)}; ALLOW_ANY_CHROME_EXTENSION_ORIGIN=${String(config.allowAnyChromeExtensionOrigin)}`,
  );
}
